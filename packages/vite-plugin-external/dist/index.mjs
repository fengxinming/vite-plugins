...
async function stash(libName, globalName, cacheDir) {
  ...
  await outputFile(libPath, makeCjsExternalCode(globalName), "utf-8");
  return libPath;
}
...
function checkLibName(externalArray, source) {
  return externalArray.some((external) => types.isRegExp(external) ? external.test(source) : external === source);
}
...
function mergeExternals(externalArray, originalExternal) {
  if (typeof originalExternal === "string" || types.isRegExp(originalExternal) || Array.isArray(originalExternal)) {
