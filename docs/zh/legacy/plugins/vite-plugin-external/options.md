...
配置外部依赖项。[示例](/zh/legacy/plugins/vite-plugin-external/usage#基础使用)
...
是否排除 Node.js 内置模块。[示例](/zh/legacy/plugins/vite-plugin-external/usage#构建时仅排除依赖)

## `externalizeDeps`
* 类型：`Array<string | RegExp>`
* 必填：`false`

指定需要排除的打包依赖项。[示例](/zh/legacy/plugins/vite-plugin-external/usage#构建时仅排除依赖)

## `externalGlobals`
* 类型：`(globals: Record<string, any>) => rollup.Plugin`
* 必填：`false`

解决 IIFE 打包问题（[Rollup Issue #3188](https://github.com/rollup/rollup/issues/3188)）。[示例](/zh/legacy/plugins/vite-plugin-external/usage#解决-iife-格式的打包问题)
...
控制 Vite 的默认构建策略调整。[示例](/zh/legacy/plugins/vite-plugin-external/usage#调整打包策略)
...
为特定模式配置外部依赖项。[示例](/zh/legacy/plugins/vite-plugin-external/usage#多模式场景配置)
