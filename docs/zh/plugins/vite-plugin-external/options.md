...
## ExternalFn 类型
* 声明形态：`(source: string, importer: string | undefined, isResolved: boolean) => string | boolean | NullValue`

**归一化决策签名（Decision hook）**：用户写的 `externals`（对象、函数、字符串、正则、数组、`true`）在内部都会先被统一转成这个签名的函数，然后在「开发预打包 / dev 中间件 / build」三个入口处共享完全相同的决策逻辑。返回值语义：
- `string`（绝对 URL）→ 走 ES CDN 重导出；
- `string`（其它）→ 走 IIFE 全局变量名；
- `true` → 纯 external（只不打包，不提供替换规则）；
- falsy → 不是 external，继续正常解析。

实际使用时直接写 `externals` 字段即可（对象/函数/正则等五种形态，见 [externals](#externals) 或 [使用示例：基础使用](/zh/plugins/vite-plugin-external/usage#基础使用)），本插件会自动做归一化。
...
## `externalGlobals`
* 类型：`(globals: ModuleNameMap) => Rolldown.Plugin`
* 必填：`false`

解决 IIFE 打包问题（[Rollup Issue #3188](https://github.com/rollup/rollup/issues/3188)）。注意 Vite 8 底层已从 Rollup 切换为 Rolldown，类型从 `rollup.Plugin` 改为 `Rolldown.Plugin`，通过 `import type { Rolldown } from 'vite'` 获取。[示例](/zh/plugins/vite-plugin-external/usage#解决-iife-格式的打包问题)

## `rollback`
* 类型：`boolean`
* 必填：`false`

是否回退到旧版实现。已废弃，Vite 8 合并实现后无效。
...
设置插件执行顺序。可选值：`pre`（前置）或 `post`（后置）。默认 `'pre'`，保证 externals 在 `@vitejs/plugin-react` 等插件之前解析。参考 [Vite 插件顺序](https://cn.vitejs.dev/guide/api-plugin.html#plugin-ordering)。
...
export interface Options extends BasicOptions {
  ...
  /**
   * 插件执行顺序（"pre" 或 "post"）
   * 默认 'pre'，保证 externals 在 React 等插件之前解析
   * @default 'pre'
   */
  enforce?: 'pre' | 'post';
   ...
   * Vite 8 使用 Rolldown 打包器，类型改为 Rolldown.Plugin
   */
  externalGlobals?: (globals: ModuleNameMap) => Rolldown.Plugin;
}
